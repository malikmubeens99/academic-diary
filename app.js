...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Ensure user is authenticated (using anonymous login for seamless instant entry, can be linked to email later)
...
function setupNavigation() {
  ...
  // Handle "View timetable" button from dashboard
...
async function loadDashboardData(userId) {
  ...
  // Render today's sample classes on dashboard
  if (todayClasses) {
    let html = "";
    sampleTimetable.slice(0, 2).forEach((cls, idx) => {
    ...
    // Attach event listeners to newly rendered diary buttons
  ...
  // Fetch count stats from Firestore collections if available
  try {
